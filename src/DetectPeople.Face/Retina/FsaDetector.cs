...
{
    public class FsaDetector
    {
        InferenceSession session1;
        InferenceSession session2;
        public void Init()
        {
            session1 = new InferenceSession("Classifiers\\fsanet-1x1-iter-688590.onnx");
            session2 = new InferenceSession("Classifiers\\fsanet-var-iter-688590.onnx");
        }
        ...
        public Tuple<int[][], float[]> GetAxis(Mat mat, Rect rect)
        {
            ...
            return new Tuple<int[][], float[]>(get_axis(yaw, pitch, roll, tdx, tdy), new float[] { yaw, pitch, roll });
        }
        float[] calcAxis(InferenceSession session, Mat mat)
        {
            mat.ConvertTo(mat, MatType.CV_32F);
            mat = mat.Resize(new OpenCvSharp.Size(64, 64));
            mat -= 127.5f;
            mat /= 128f;
