...
namespace DetectPeople.Face.Retina
{
    public class RetinaFace
    {
        ...
        public Tuple<RectangleF[], Point2f[][], float[]> Forward(Mat mat, InferenceSession session)
        {
            var origWidth = mat.Width;
            var origHeight = mat.Height;
            ...
            List<OpenCvSharp.Point2f[]> olandms = new List<OpenCvSharp.Point2f[]>();
            ...
            for (var i = 0; i < dets.Count(); i++)
            {
                ...
                detections.Add(new RectangleF(aa[0], aa[1], aa[2] - aa[0], aa[3] - aa[1]));
                indexMap.Add(i);

                List<Point2f> l = new List<Point2f>();
                for (int j = 0; j < landms[i].Length; j += 2)
                {
                    l.Add(new Point2f(landms[i][j], landms[i][j + 1]));
                }
            ...
            return new Tuple<RectangleF[], Point2f[][], float[]>(detections.ToArray(), olandms.ToArray(), oscores.ToArray());
        }
